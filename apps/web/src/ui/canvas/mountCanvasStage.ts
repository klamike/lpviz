...
export function mountCanvasStage(parent: HTMLElement, ctx: AppContext, onResizeStart: (event: PointerEvent) => void) {
  ...
  reset.innerHTML = '<svg width="25" height="25" viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" /></svg>';
  reset.addEventListener("click", () => ctx.actions.resetView());
  const fit = el("button", { attrs: { title: "Zoom" } });
  fit.innerHTML = '<svg width="25" height="25" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><defs><mask id="hole-mask"><rect width="100" height="100" fill="white" /><circle cx="40" cy="40" r="20" fill="black" /></mask></defs><circle cx="40" cy="40" r="32.5" mask="url(#hole-mask)" /><g transform="translate(55,55) rotate(45)"><rect x="0" y="-4" width="52.5" height="15" /></g></svg>';
  ...
  main.append(handle);
  function render() {
    const { is3DMode, zScale, problemMode } = getState();
    // in /3d the button links back to the 2-variable app, and z is a real
    // coordinate rather than a scalable visualization lift (no zScale slider)
    const is3DProblem = problemMode === "3d";
    toggle3d.className = is3DMode && !is3DProblem ? "button-active" : "";
    toggle3d.textContent = is3DProblem || is3DMode ? "2D" : "3D";
    toggle3d.title = is3DProblem ? "Back to 2-variable mode" : "Toggle 3D Mode";
    zs.value = String(zScale);
    zv.textContent = zScale.toFixed(2);
    zc.className = is3DMode && !is3DProblem ? "" : "is-hidden";
