...
function assertCloseVector(label: string, actual: ArrayLike<number>, expected: ArrayLike<number>, tol = TOLERANCE) {
  if (actual.length !== expected.length) {
    throw new Error(`${label}: vector length mismatch ${actual.length} !== ${expected.length}`);
  }
  for (let i = 0; i < actual.length; i++) {
    const delta = Math.abs(actual[i]! - expected[i]!);
    if (delta > tol) {
      throw new Error(`${label}: index ${i} differs by ${delta}; actual=${actual[i]}, expected=${expected[i]}`);
    }
  }
}
...
function validateFixture() {
  ...
  if (problem20.vertices.length < 3) throw new Error("fixture must have vertices");
}
...
function runTypedCentral(vertices: Vertices, lines: Lines, objective: Float64Array) {
  return centralPath(vertices, lines, objective, { niter: CENTRAL_PATH_POINTS, verbose: false });
}

function runNumberCentral(vertices: Vertices, lines: Lines, objective: Float64Array) {
  return centralPathNumber(vertices, lines, objective, { niter: CENTRAL_PATH_POINTS, verbose: false });
}
...
function runNumberSimplex(lines: Lines, objective: Float64Array, dual: boolean) {
  return simplexNumber(lines, objective, { tol: TOLERANCE, verbose: false, dual });
}
...
function main() {
  ...
  results.push(runBench({
    ...
    assertEquivalent: (typedResult, numberResult) => {
      assertEqual("central iteration count", typedResult.iterations.length, numberResult.iterations.length);
      assertCloseVector("central final point", finalPoint(typedResult.iterations), finalPoint(numberResult.iterations), 5e-5);
    },
  }));
  ...
  results.push(runBench({
    ...
    assertEquivalent: (typedResult, numberResult) => {
      ...
      assertEqual("ipm iteration count", typedIterations.length, numberIterations.length);
      assertCloseVector("ipm final point", finalPoint(typedIterations), finalPoint(numberIterations), 5e-5);
    },
  }));
  ...
  for (const dual of [false, true]) {
    results.push(runBench({
      ...
      assertEquivalent: (typedResult, numberResult) => {
        assertEqual(`${dual ? "dual" : "primal"} simplex mode`, typedResult.mode, numberResult.mode);
        assertEqual(`${dual ? "dual" : "primal"} simplex status`, typedResult.status, numberResult.status);
        assertEqual(`${dual ? "dual" : "primal"} simplex iteration count`, typedResult.iterations.length, numberResult.iterations.length);
        if (typedResult.iterations.length > 0 || numberResult.iterations.length > 0) {
          assertCloseVector(`${dual ? "dual" : "primal"} simplex final point`, finalPoint(typedResult.iterations), finalPoint(numberResult.iterations), 5e-5);
        }
      },
    }));
  }
  ...
  for (const ineq of [false, true]) {
    results.push(runBench({
      ...
      assertEquivalent: (typedResult, numberResult) => {
        assertEqual(`${ineq ? "ineq" : "eq"} pdhg iteration count`, typedResult.iterations.length, numberResult.iterations.length);
        assertCloseVector(`${ineq ? "ineq" : "eq"} pdhg final point`, finalPoint(typedResult.iterations), finalPoint(numberResult.iterations), 5e-5);
      },
    }));
  }
