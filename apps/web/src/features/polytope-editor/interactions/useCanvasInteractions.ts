import type { HandleUndoRedo, SaveHistory } from "@/features/history/useHistory";
...
export function useCanvasInteractions({
  ...
  useEffect(() => {
    ...
    const persistPendingDragHistory = () => {
      if (!pendingDragHistoryRef.current) return;
      saveHistoryRef.current(pendingDragHistoryRef.current);
    ...
    const handleDragStart = (clientX: number, clientY: number): boolean => {
      ...
      if (target.kind === "objective") {
        pendingDragHistoryRef.current = captureHistoryEntry(state);
        setState(
          {
    ...
    const handleDragMove = (clientX: number, clientY: number) => {
      const logicalCoords = getLogicalFromClient(
        canvasManager,
        clientX,
        clientY,
      );
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "z") {
        event.preventDefault();
        handleUndoRedoRef.current(event.shiftKey);
      }
      if (event.key === "Enter") {
        event.preventDefault();
        finishOpenRegion();
