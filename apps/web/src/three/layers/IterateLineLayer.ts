import { isRenderBenchComputeLineDistances, isRenderBenchForceAllDirty, isRenderBenchLegacyBounds } from "@/bench/renderBenchConfig";
...
function getIterateRenderZ(entry: Float64Array, objectiveVector: PointXY | null, zScale: number, is3D: boolean, transitionZMultiplier = 1) {
  if (!is3D) return 0;
  return ((computeIterateZ(entry, objectiveVector) * zScale) / 100) * transitionZMultiplier;
}

function buildLinePositions(path: Float64Array[], objectiveVector: PointXY | null, zScale: number, is3D: boolean, transitionZMultiplier = 1) {
...
function buildIterateSegments(raw: State, is3D: boolean, transitionZMultiplier = 1): SegmentEntry[] {
  ...
  const hasPhases = raw.iteratePhases.length === raw.iteratePath.length && raw.iteratePhases.length > 0;

  if (!hasPhases) {
    const positions = buildLinePositions(raw.iteratePath, raw.iterateObjectiveVector, raw.zScale, is3D, transitionZMultiplier);
...
function makeLine2(group: Group): Line2 {
  ...
  ln.frustumCulled = isRenderBenchLegacyBounds();
  if (!isRenderBenchComputeLineDistances()) {
...
export class IterateLineLayer implements Layer {
  ...
  update(ctx: SceneContext): void {
    ...
    const p = this.prev;
    if (!isRenderBenchForceAllDirty() && p && p.iteratePath === raw.iteratePath && p.iteratePhases === raw.iteratePhases && p.iterateObjectiveVector === raw.iterateObjectiveVector && p.zScale === raw.zScale && p.is3DMode === raw.is3DMode && p.isTransitioning3D === raw.isTransitioning3D && p.mode === snap.mode && p.transitionZMultiplier === snap.transitionZMultiplier) {
      return;
    }
    this.prev = {
