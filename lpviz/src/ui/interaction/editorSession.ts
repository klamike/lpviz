import { computeDrawingPhase } from "../../state/store";
import type { CompletionMode, State } from "../../state/store";
import type { PointXY } from "../../solvers/utils/blas";
import { deriveRegionFromPoints } from "../../solvers/utils/regionAssembly";
import { centroid, isConvexChain, signedArea, VRep } from "../../solvers/utils/polygon";
...
export function getEditorContext(state: State) {
  ...
  const isDraggingGeometry =
    state.editorInteraction.kind === "dragging" && state.editorInteraction.target.kind !== "objective";
  const geometry =
...
export function computeEditorRegionForState(state: State) {
  ...
  if (sourceMode === "open" && isDraggingGeometry && !isConvexChain(sourceVertices)) {
  ...
  const isConvex =
    sourceMode === "open"
      ? region.kind !== "bounded"
        ...
        : VRep.fromPoints(region.vertices.map(([x, y]) => ({ x, y }))).isConvex()
      : vertexRep.isConvex();
  ...
  switch (action.kind) {
    ...
    case "delete-vertex": {
      ...
      if (isDerivedClosed || session.kind === "editing-closed") {
        ...
        const reopenedVertices = Array.from({ length: nextVertices.length }, (_, offset) => {
          const sourceIndex = (action.deleteIndex + 1 + offset) % displayVertices.length;
          return displayVertices[sourceIndex];
        });
        const orientedVertices = signedArea(displayVertices) > 0 ? reopenedVertices.reverse() : reopenedVertices;
