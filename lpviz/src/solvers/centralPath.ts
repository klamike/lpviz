...
import { dot, infinityNorm, linesToDenseAb, matVec, solveDenseSystem } from "./utils/dense";
import type { Lines, Vertices, VecN, VecNs } from "./utils/blas";
import { findStrictFeasiblePoint } from "./utils/feasibleRegion";
...
function centralPathXk(
  ...
  for (let iteration = 1; iteration <= maxit; iteration++) {
    ...
    if (verbose) {
      ...
      console.log(sprintf("Iter %d: f(x) = %.6f, ||grad||_inf = %.2e, alpha = %.2f", iteration, objectiveValue, gradientInfinityNorm, stepSize));
    }
  }
...
export function centralPath(vertices: Vertices, lines: Lines, objective: VecN, opts: CentralPathOptions) {
  ...
  const points: VecNs = [];
  const logs: string[] = [];
  const header = sprintf("  %-4s %8s %8s %10s %10s  \n", "Iter", "x", "y", "Obj", "µ");
  ...
  for (const mu of barrierParameters) {
    ...
    const progressLog = sprintf("  %-4d %+8.2f %+8.2f %+10.1e %10.1e  \n", points.length, optimalPoint[0] ?? 0, optimalPoint[1] ?? 0, linearObjective, mu);
