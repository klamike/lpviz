...
export type OnboardingUiController = {
  ...
  showHelpPopup: (options: Omit<PopupOptions, "id" | "side">) => void;
  hideHelpPopup: () => void;
  showNonconvexHint: (options: Omit<PopupOptions, "id" | "side">) => void;
  hideNonconvexHint: () => void;
};

type OnboardingUiContextValue = {
  controller: OnboardingUiController;
  registerActionTarget: (target: OnboardingActionTarget, element: HTMLElement | null) => void;
};
  ...
  return (
    ...
    >
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
        <path d="M3 3l7.07 16.97 2.51-7.39 7.39-2.51L3 3z" fill="#4A90E2" stroke="#fff" strokeWidth="1.5" />
      </svg>
    </div>
  );
}
...
export function OnboardingUiProvider({ children }: PropsWithChildren) {
  const actionTargetsRef = useRef<Partial<Record<OnboardingActionTarget, HTMLElement>>>({});
  const popupTimersRef = useRef<{ help: number | null; nonconvex: number | null }>({
    help: null,
    nonconvex: null,
  });
  ...
  const registerActionTarget = useCallback((target: OnboardingActionTarget, element: HTMLElement | null) => {
    if (element) {
  ...
  const showPopup = useCallback((
    ...
    const timer = kind === "help" ? popupTimersRef.current.help : popupTimersRef.current.nonconvex;
    if (timer !== null) {
      clearTimeout(timer);
      if (kind === "help") {
  ...
  const hidePopup = useCallback((kind: "help" | "nonconvex") => {
    const setPopup = kind === "help" ? setHelpPopup : setNonconvexPopup;
    const currentTimer = kind === "help" ? popupTimersRef.current.help : popupTimersRef.current.nonconvex;
    ...
    setPopup((current) => {
      ...
      const timer = window.setTimeout(() => {
        setPopup((latest) => (latest && latest.token === token ? null : latest));
        if (kind === "help") {
          popupTimersRef.current.help = null;
        } else {
          popupTimersRef.current.nonconvex = null;
...
export function useOnboardingActionTarget<T extends HTMLElement = HTMLElement>(target: OnboardingActionTarget) {
  const context = useContext(OnboardingUiContext);
  if (!context) {
    throw new Error("OnboardingUiProvider is missing");
  }
