...
export function setupUIControls(
    ...
    const solverButtons = [
      {
        ...
        mode: "central" as SolverMode,
        settings: "centralPathSettings",
      },
  ...
  function setupInputWithSolverMode(
    inputId: string,
    solverMode: string,
  ...
  function setupSettingsInputs() {
    ...
    const sliderElements = sliders.reduce(
      (acc, config) => {
        acc[config.id] = setupSliderWithDisplay(
          config.id,
          config.displayId,
          config.decimals,
          config.solver,
    ...
    const objectiveAngleStepSlider = setupSliderWithDisplay(
      "objectiveAngleStepSlider",
      "objectiveAngleStepValue",
      2,
      undefined,
      () => {
        if (state.traceEnabled) {
    ...
    const inputElements = inputs.reduce(
      (acc, config) => {
        acc[config.id] = setupInputWithSolverMode(
          config.id,
          config.solver,
          config.eventType,
        );
  ...
  function setupActionButtons() {
    // Trace/Solve Button
    getElement<HTMLButtonElement>("traceButton").addEventListener(
      "click",
      () => {
        computePath();
    ...
    const startRotateButton = getElement<HTMLButtonElement>(
      "startRotateObjectiveButton",
    );
    const stopRotateButton = getElement<HTMLButtonElement>(
      "stopRotateObjectiveButton",
    );
    const rotationSettings = getElement<HTMLElement>(
    ...
    switch (state.solverMode) {
      case "ipm":
        await computeIPMSolution(
          settingsElements.alphaMaxSlider,
          settingsElements.maxitInput,
          updateResult,
        );
        break;
      case "simplex":
      ...
      default: // central
        await computeCentralPathSolution(
          settingsElements.centralPathIterSlider,
          settingsElements.objectiveAngleStepSlider,
          updateResult,
        );
        break;
    }
  ...
  async function computeAndRotate() {
    ...
    const angleStep = parseFloat(
      settingsElements.objectiveAngleStepSlider.value,
    );
