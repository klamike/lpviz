...
import {
  inverseTransform2DProjection,
  transform2DTo3DAndProject,
} from "../utils/math3d";
...
export class CanvasManager {
  ...
  toLogicalCoords(x: number, y: number) {
    if (state.is3DMode || state.isTransitioning3D) {
      let projected2D = {
        x:
          (x - this.centerX) / (this.gridSpacing * this.scaleFactor) -
          this.offset.x,
        y:
          (this.centerY - y) / (this.gridSpacing * this.scaleFactor) -
          this.offset.y,
      };

      let logical = inverseTransform2DProjection(
        projected2D,
        state.viewAngle,
        state.focalDistance,
      );
  ...
  toCanvasCoords(x: number, y: number, z?: number) {
    if (state.is3DMode || state.isTransitioning3D) {
      ...
      return {
        x:
          this.centerX +
          (projected.x + this.offset.x) * this.gridSpacing * this.scaleFactor,
        y:
          this.centerY -
          (projected.y + this.offset.y) * this.gridSpacing * this.scaleFactor,
      };
    } else {
      return {
        x:
          this.centerX +
          (x + this.offset.x) * this.gridSpacing * this.scaleFactor,
        y:
          this.centerY -
          (y + this.offset.y) * this.gridSpacing * this.scaleFactor,
      };
    }
  }
  ...
  drawGrid() {
    ...
    if (state.is3DMode || state.isTransitioning3D) {
      ...
      const originX =
        this.centerX + this.offset.x * this.gridSpacing * this.scaleFactor;
      const originY =
        this.centerY - this.offset.y * this.gridSpacing * this.scaleFactor;
      this.ctx.strokeStyle = "#e0e0e0";
      this.ctx.lineWidth = 0.5;
      for (
        let x = originX;
        x <= width;
        x += this.gridSpacing * this.scaleFactor
      ) {
        this.ctx.beginPath();
        this.ctx.moveTo(x, 0);
      ...
      for (
        let y = originY;
        y <= height;
        y += this.gridSpacing * this.scaleFactor
      ) {
        this.ctx.beginPath();
        this.ctx.moveTo(0, y);
  ...
  drawPolygon() {
    ...
    if (state.polygonComplete) {
      if (state.vertices.length >= 3 && state.inputMode !== "manual") {
        ...
        this.ctx.beginPath();
        const start = this.toCanvasCoords(
          state.vertices[0].x,
          state.vertices[0].y,
        );
        this.ctx.moveTo(start.x, start.y);
        for (let i = 1; i < state.vertices.length; i++) {
          const cp = this.toCanvasCoords(
            state.vertices[i].x,
            state.vertices[i].y,
          );
      ...
      if (state.currentMouse) {
        const rbEnd = this.toCanvasCoords(
  ...
  drawConstraintLines() {
    if (
      ...
      state.computedLines.forEach((line, index) => {
        ...
        const bottomRight = this.toLogicalCoords(
          width + margin,
          height + margin,
        );
        ...
        this.ctx.strokeStyle =
          state.highlightIndex === index ? "red" : "rgba(100, 100, 100, 0.7)";
  ...
  drawObjective() {
    const target =
      ...
      (state.polygonComplete && state.currentObjective
  ...
  drawSingleTrace(path: number[][]) {
    ...
    const startZ =
      path[0][2] !== undefined
        ? path[0][2]
        : state.objectiveVector
          ? state.objectiveVector.x * path[0][0] +
            state.objectiveVector.y * path[0][1]
          : 0;
    ...
    for (let i = 1; i < path.length; i++) {
      const z =
        path[i][2] !== undefined
          ? path[i][2]
          : state.objectiveVector
            ? state.objectiveVector.x * path[i][0] +
              state.objectiveVector.y * path[i][1]
            : 0;
    ...
    path.forEach((entry: number[]) => {
      const z =
        entry[2] !== undefined
          ? entry[2]
          : state.objectiveVector
            ? state.objectiveVector.x * entry[0] +
              state.objectiveVector.y * entry[1]
            : 0;
  ...
  drawIteratePath() {
    ...
    if (state.iteratePath && state.iteratePath.length > 0) {
      ...
      const startZ =
        state.iteratePath[0][2] !== undefined
          ? state.iteratePath[0][2]
          : state.objectiveVector
            ? state.objectiveVector.x * state.iteratePath[0][0] +
              state.objectiveVector.y * state.iteratePath[0][1]
            : 0;
      const start = this.toCanvasCoords(
        state.iteratePath[0][0],
        state.iteratePath[0][1],
        startZ,
      );
      this.ctx.moveTo(start.x, start.y);
      ...
      for (let i = 1; i < state.iteratePath.length; i++) {
        const z =
          state.iteratePath[i][2] !== undefined
            ? state.iteratePath[i][2]
            : state.objectiveVector
              ? state.objectiveVector.x * state.iteratePath[i][0] +
                state.objectiveVector.y * state.iteratePath[i][1]
              : 0;
        const pt = this.toCanvasCoords(
          state.iteratePath[i][0],
          state.iteratePath[i][1],
          z,
        );
        this.ctx.lineTo(pt.x, pt.y);
      }
      ...
      state.iteratePath.forEach((entry, i) => {
        ...
        if (i === state.iteratePath.length - 1) {
          ...
          this.ctx.beginPath();
          this.ctx.fillStyle =
            state.highlightIteratePathIndex === i ? "green" : "purple";
          this.ctx.arc(
            cp.x,
            cp.y,
            state.highlightIteratePathIndex === i ? 5 : 3,
            0,
            2 * Math.PI,
          );
          this.ctx.fill();
