...
} from "../../../polytope/objectiveDirection";
...
import type { ResultTextBlock } from "../types/resultPayload";
...
export function createSolverRuntime({
  ...
  resultRuntime: {
    render: (
      ...
      options?: { limitVirtualRows?: boolean },
    ) => void;
  ...
  const runtime = {
    ...
    advanceRotationClock(timestamp: number) {
      ...
      if (
        !this.rotationComputeInFlight &&
        this.rotationElapsedMs >= intervalMs
      ) {
        this.rotationElapsedMs = 0;
    ...
    ensureRotationLoop() {
      if (!getState().rotateObjectiveMode || this.rotationRafId !== null)
        return;
    ...
    async computePath() {
      ...
      try {
        ...
        solverDefinition.applyResult(response, (payload) =>
          resultRuntime.render(payload),
        );
        canvasManager.draw();
      } catch (error) {
        if (requestGeneration !== this.requestGeneration) {
    ...
    setTraceEnabled(enabled: boolean) {
      setState(
        ...
        { viewportDirty: canvasManager.getTraceDirtyFlags() },
      );
      if (!enabled) {
        resetTraceState();
        canvasManager.draw();
