...
import { verticesFromLines } from "../../../polytope/halfPlaneIntersection";
...
export function registerCanvasInteractions(
  ...
  const interactionController = {
    ...
    getDragStartTarget(
      ...
      if (session.kind === "drafting") {
        const index = this.findVertexNearLocalPoint(
          local.x,
          local.y,
          state.vertices,
        );
      ...
      if (state.objectiveVector) {
        const tip = canvasManager.getObjectiveScreenPosition(
          state.objectiveVector,
        );
      ...
      const vertexIndex = this.findVertexNearLocalPoint(
        local.x,
        local.y,
        state.vertices,
      );
      ...
      if (session.kind === "editing-open" && state.vertices.length >= 2) {
        ...
        const edgeIndex = this.findEdgeNearPoint(
          logicalCoords,
          state.vertices,
          "open",
        );
    ...
    applyEditorTransition(
      ...
      if (transition.kind === "edit") {
        this.commitEdit(transition.result, {
    ...
    handleDragMove(clientX: number, clientY: number) {
      ...
      if (
        ...
        this.exceedsDragThreshold(clientX, clientY)
      ) {
        setState(
          {
            editorInteraction: {
    ...
    handleContextMenu(event: MouseEvent) {
      ...
      const deleteIndex = this.findVertexNearLocalPoint(
        local.x,
        local.y,
        displayVertices,
      );
    ...
    handleDoubleClick(event: MouseEvent) {
      ...
      const logicalMouse = this.getLogicalFromClient(
        event.clientX,
        event.clientY,
      );
      const state = getState();
  ...
  bindEvent(
    ...
    (event: MouseEvent) => {
      ...
      interactionController.handlePointerStart(
        event.clientX,
        event.clientY,
        event,
      );
  ...
  bindEvent(
    ...
    "mousemove",
    (event: MouseEvent) => {
      interactionController.handlePointerMove(
        event.clientX,
        event.clientY,
        event,
      );
  ...
  bindEvent(canvas, "dblclick", (event: MouseEvent) =>
    interactionController.handleDoubleClick(event),
  );

  bindEvent(canvas, "click", (event: MouseEvent) =>
