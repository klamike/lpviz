...
import type { Line, PointXY, PointXYZ } from "./math/blas";
...
export class ViewportManager {
  ...
  private constructor(canvas: HTMLCanvasElement) {
    ...
    this.scenes.foreground.add(
      this.groups.polytopeOutline,
      this.groups.constraint,
    ...
    this.controls.orbit.addEventListener(
      "change",
      this.handleOrbitControlsChange,
    );
    ...
    this.controls.ortho.addEventListener(
      "change",
      this.handleOrthoControlsChange,
    );
    ...
    this.renderHelpers = {
      ...
      createThickLine: (positions, options) =>
        this.createThickLine(positions, options),
      getWorldSizeFromPixels: (pixels, worldPosition) =>
        this.getWorldSizeFromPixels(pixels, worldPosition),
  ...
  destroy() {
    ...
    this.controls.orbit.removeEventListener(
      "change",
      this.handleOrbitControlsChange,
    );
    this.controls.ortho.removeEventListener(
      "change",
      this.handleOrthoControlsChange,
    );
  ...
  private invalidateScene(parts: Partial<typeof this.renderState.dirty> = {}) {
    const entries = Object.entries(parts) as Array<
      [keyof typeof this.renderState.dirty, boolean | undefined]
    >;
    if (entries.length === 0) {
  ...
  private captureStateSignatures(state: State) {
    const polytopeLines =
      state.polytope && hasPolytopeLines(state.polytope)
        ? state.polytope.lines.map((line) => line.join(",")).join(";")
        : "";
    const polytopeVertices =
      state.polytope?.kind === "bounded"
        ? state.polytope.vertices.map((vertex) => vertex.join(",")).join(";")
        : "";
    const boundaryRays =
      state.polytope?.boundaryRays
        ?.map((ray) => `${ray.start.join(",")}|${ray.direction.join(",")}`)
        .join(";") ?? "";
    const transitionKey = state.isTransitioning3D
      ? `${state.transitionDirection}:${state.transitionProgress}:${state.viewAngle.x},${state.viewAngle.y},${state.viewAngle.z}`
      : "";
    ...
    return {
      ...
      trace: [
        ...
        state.traceBuffer
          .map(
            (entry) =>
              `${entry.objectiveVector ? `${entry.objectiveVector.x},${entry.objectiveVector.y}` : ""}:${this.serializeNumberPath(entry.path)}`,
          )
          .join("|"),
      ].join("|"),
      iterate: [
  ...
  private invalidateFromStateChange(state: State) {
    ...
    if (next.constraints !== this.stateSignatures.constraints)
      dirty.constraints = true;
    if (next.objective !== this.stateSignatures.objective)
      dirty.objective = true;
  ...
  private buildArrowHeadSegments(
    tip: PointXY,
    angle: number,
    length: number,
  ...
  private getPlanarViewportTarget() {
    ...
    const centerShiftX =
      (this.viewState.centerX - window.innerWidth / 2) * unitsPerPixel;
    const centerShiftY =
      (this.viewState.centerY - window.innerHeight / 2) * unitsPerPixel;
    return new Vector3(
  ...
  private getPerspectiveDistanceToFitBounds(
    ...
    const availWidth = Math.max(
      100,
      window.innerWidth - this.sidebarWidth - 2 * padding,
    );
  ...
  private getVisibleBounds(
    ...
    const bottomRight = toLogicalCoords(
  ...
  private getProjectionViewAngle() {
    return this.controlState.orbitActive
      ? this.getOrbitAngles(this.cameras.perspective.rotation)
      : getState().viewAngle;
  }
  ...
  private getDisplayedZValue(
    x: number,
    y: number,
    z?: number,
    objectiveVector?: PointXY | null,
  ) {
    return getDisplayedIterateZ(
  ...
  private getPerspectiveDistance(
    unitsPerPixel = this.getUnitsPerPixel(),
    height = window.innerHeight,
  ) {
  ...
  private getMaxPerspectiveDistance() {
    return this.getPerspectiveDistance(
      1 / (this.viewState.gridSpacing * ORTHO_MIN_SCALE_FACTOR),
    );
  }
  ...
  private getScaleFactorFromDistance(
    distance: number,
    height = window.innerHeight,
  ) {
    const fov = this.cameras.perspective.fov * (Math.PI / 180);
    ...
    return this.clampScaleFactor(
      1 / (unitsPerPixel * this.viewState.gridSpacing),
    );
  }
  ...
  private projectScreenToInteractionPlane(
    screenX: number,
    screenY: number,
  ): PointXY | null {
    ...
    this.interactionProjection.pointerNdc.set(
      (screenX / width) * 2 - 1,
      -((screenY / height) * 2 - 1),
    );
    this.cameras.perspective.updateMatrixWorld();
    this.cameras.perspective.updateProjectionMatrix();
    this.interactionProjection.raycaster.setFromCamera(
      this.interactionProjection.pointerNdc,
    ...
    return this.interactionProjection.raycaster.ray.intersectPlane(
      ...
      ? {
          x: this.interactionProjection.pointerWorld.x,
          y: this.interactionProjection.pointerWorld.y,
        }
      : null;
  }

  private projectScreenToWorldPlane(
    screenX: number,
    screenY: number,
    z = 0,
  ): PointXY | null {
  ...
  private getVisibleCenterWorldPoint(z = 0) {
    const point = this.projectScreenToWorldPlane(
  ...
  private getCurrentTransitionTarget() {
    ...
    return this.controlState.transitionStartTarget
      .clone()
      .lerp(this.controlState.transitionEndTarget, state.transitionProgress);
  }
  ...
  private getFlattenTo2DProgress() {
    ...
    return state.transitionDirection === "to3d"
      ? 1 - state.transitionProgress
  ...
  private align2DStateToCurrentTransitionView() {
    if (
      !(
        getState().isTransitioning3D &&
        getState().transitionDirection === "to2d"
      )
    ) {
      return;
    }
    const returnFocus = this.getVisibleCenterWorldPoint(
      this.controlState.transitionEndTarget.z,
    );
  ...
  private initializeTransitionTargets(targetMode: boolean) {
    if (targetMode) {
      ...
      this.controlState.transitionEndTarget.copy(
        this.controlState.transitionStartTarget,
      );
      if (this.controlState.orbitActive) {
    ...
    this.syncOffsetFromTarget(
      this.controlState.transitionEndTarget.x,
      this.controlState.transitionEndTarget.y,
    );
  ...
  private getBlendedObjectiveZ(
    ...
    return this.getBlendedRenderZ(
      this.getDisplayedZValue(x, y),
      planarOffset,
      context,
    );
  }
  ...
  private getBlendedPointPosition(
    ...
    return new Vector3(
      x,
      y,
      this.getBlendedRenderZ(zValue, planarOffset, context),
    );
  }

  private getVertexZ(x: number, y: number, extra = 0) {
    return (
      this.scaleZValue(this.getDisplayedZValue(x, y)) +
      this.getPlanarOffset(extra)
    );
  }
  ...
  private getObjectiveWorldPosition(target: PointXY) {
    return new Vector3(
      target.x,
      target.y,
      this.getPlanarOffset(OBJECTIVE_Z_OFFSET),
    );
  }
  ...
  private toPlaneVector(screenX: number, screenY: number) {
    return new Vector2(
      (screenX - this.viewState.centerX) * this.getUnitsPerPixel() -
        this.viewState.offset.x,
      (this.viewState.centerY - screenY) * this.getUnitsPerPixel() -
        this.viewState.offset.y,
    );
  }
  ...
  private inverseProject2DPoint(
    projectedPoint2d: PointXY,
    viewAngles: PointXYZ,
  ): PointXY {
    const inverseRotationMatrix = new Matrix4()
      .makeRotationFromEuler(
        new Euler(viewAngles.x, viewAngles.y, viewAngles.z, "XYZ"),
      )
      .invert();
  ...
  private buildPositionVector(entry: number[], planarOffset = 0) {
    ...
    return new Vector3(
      x,
      y,
      this.scaleZValue(zValue) + this.getPlanarOffset(planarOffset),
    );
  }
  ...
  private applyPerspectivePose(
    viewAngle: PointXYZ,
    distance = this.getDefaultPerspectiveDistance(),
    target = this.getViewportTarget(),
  ) {
    const euler = new Euler(-viewAngle.x, -viewAngle.y, -viewAngle.z, "XYZ");
  ...
  private captureReturnScaleFrom3D() {
    const target = this.controlState.orbitActive
      ? this.controls.orbit.target
      : this.getViewportTarget();
  ...
  private snapshotOrbitState() {
    ...
    const distance = this.clampPerspectiveDistance(
      this.cameras.perspective.position.distanceTo(this.controls.orbit.target),
    );
    this.controlState.currentPerspectiveDistance = Number.isFinite(distance)
      ? distance
      : this.controlState.currentPerspectiveDistance;
  ...
  private getPointMaterial(options: {
    ...
    const texture =
      shape === "square" ? this.getSquareTexture() : this.getCircleTexture();
    const key = [
      shape,
      options.color,
      options.size,
  ...
  private getSpriteMaterial(type: "circle" | "star" | "square", color: number) {
    const texture =
      type === "star"
        ? this.getStarTexture(color)
        : type === "square"
          ? this.getSquareTexture()
          : this.getCircleTexture();
  ...
  private getOrCreatePolytopeVertexSprite(
    index: number,
    type: "circle" | "square",
    color: number,
    pixelSize: number,
  ...
  private updatePointCloudGeometry(
    pointCloud: Points,
    ...
    geometry.setAttribute(
      "position",
      new Float32BufferAttribute(Array.from(positions), 3),
    );
    if (colors) {
      geometry.setAttribute(
        "color",
        new Float32BufferAttribute(Array.from(colors), 3),
      );
  ...
  private createCircleSprite(
    ...
    return this.createSpriteAtSize(
      position,
  ...
  private updateLineSegmentsGeometry(
  ...
  private renderFrame() {
    ...
    [
      ...
      this.scenes.vertices,
      this.scenes.traceLines,
      this.scenes.trace,
      this.scenes.overlay,
    ].forEach((scene) => this.renderer.render(scene, this.cameras.active));
    this.renderer.autoClear = true;
  }
  ...
  private renderGrid(
    ...
    this.updateLineSegmentsGeometry(this.gridObjects.axes, [
      0,
      minY,
      0,
      0,
  ...
  private renderPolytope(
    context: ReturnType<ViewportManager["buildRenderContext"]>,
  ) {
    const { is3D, skipPreviewDrawing } = context;

    const { vertices, completionMode, highlightIndex, currentMouse, polytope } =
      getState();
    ...
    const displayVertices: PointXY[] = hasDerivedClosedRegion
      ...
      : vertices;
    const isClosedRegion =
      completionMode === "closed" || hasDerivedClosedRegion;
    ...
    const useFixedUnboundedBounds =
      completionMode === "open" &&
      !hasDerivedClosedRegion &&
      polytope?.kind === "unbounded";
    const bounds = useFixedUnboundedBounds
  ...
  private renderConstraints(
    ...
    polytope.lines.forEach((line, index) => {
      ...
      this.updateThickLine(
        this.getOrCreateConstraintLine(),
  ...
  private renderObjective(
    ...
    const arrowColor =
      polytope?.kind === "unbounded" &&
      hasPolytopeLines(polytope) &&
      isObjectiveDirectionUnbounded(polytope.lines, [target.x, target.y])
        ? COLORS.polytopeHighlight
        : COLORS.objective;
    let objectiveLineCount = 0;
    this.updateThickLine(
      this.getOrCreateObjectiveLine(objectiveLineCount++),
    ...
    for (
      let index = objectiveLineCount;
      index < this.persistentSceneObjects.objectiveLines.length;
      index++
    ) {
  ...
  private buildTraceLinePositions(
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      const scaledZ = context.scaleZValue(
        getDisplayedIterateZ(entry, objectiveVector),
      );
  ...
  private buildTraceSamplePositions(linePositions: number[]): number[] {
    ...
    if (
      ...
      samples.push(
        linePositions[lastBaseIndex],
        linePositions[lastBaseIndex + 1],
        linePositions[lastBaseIndex + 2],
      );
    }
    return samples;
  }
  ...
  private createPointCloud(
    ...
    if (options.colors) {
      geometry.setAttribute(
        "color",
        new Float32BufferAttribute(Array.from(options.colors), 3),
      );
    }
    const material = helpers.getPointMaterial({
      color: options.color,
  ...
  private renderIterate(
    ...
    if (!iteratePath || iteratePath.length === 0) {
      ...
      if (this.persistentSceneObjects.iteratePoints)
        this.persistentSceneObjects.iteratePoints.visible = false;
      if (this.persistentSceneObjects.iterateRestartPoints)
    ...
    for (let i = 0; i < iteratePath.length; i++) {
      ...
      positions[i * 3 + 2] = z;
      const colorValue = hasPhases
        ? PHASE_COLORS[iteratePhases[i]! % PHASE_COLORS.length]
        : COLORS.iteratePath;
    ...
    if (visibleRestartIndices.length > 0) {
      ...
      const iterateRestartPoints = this.getOrCreatePointCloud(
        this.persistentSceneObjects.iterateRestartPoints,
        groups.iterate,
        helpers,
        {
          color: COLORS.iteratePath,
    ...
    if (
      ...
      const highlightSprite =
        this.persistentSceneObjects.iterateHighlight ??
  ...
  private handleOrthoControlsChange = () => {
    ...
    if (Number.isFinite(zoomValue) && zoomValue !== 1) {
      this.viewState.scaleFactor = this.clampScaleFactor(
        this.viewState.scaleFactor * zoomValue,
      );
      this.cameras.ortho.zoom = 1;
      this.cameras.ortho.updateProjectionMatrix();
    }
    ...
    this.syncOffsetFromTarget(
      this.controls.ortho.target.x,
      this.controls.ortho.target.y,
    );
  ...
  private handleOrbitControlsChange = () => {
    ...
    if (
      this.controlState.lastOrbitTarget.distanceToSquared(
        this.controls.orbit.target,
      ) >= 1e-9
    ) {
      this.controlState.lastOrbitTarget.copy(this.controls.orbit.target);
    }
    const distance = this.cameras.perspective.position.distanceTo(
      this.controls.orbit.target,
    );
    this.controlState.currentPerspectiveDistance = Number.isFinite(distance)
      ? distance
    ...
    this.invalidateScene({
      ...
      objective:
        Math.abs(
          this.controlState.currentPerspectiveDistance - previousDistance,
        ) > 1e-6,
      trace: false,
      iterate: false,
    });
  ...
  resetView() {
    if (this.is3DState()) {
      ...
      const scaleFactor = 1;
      const distance = this.getPerspectiveDistance(
        1 / (this.viewState.gridSpacing * scaleFactor),
      );
  ...
  toLogicalCoords(x: number, y: number): PointXY {
    ...
    if (is3D) {
      ...
      this.updateInteractionPlane(
        projectionState.objectiveVector,
        projectionState.zScale,
        is3D,
      );
  ...
  private clamp3DInteractionPoint(point: PointXY): PointXY {
    ...
    if (
      !(
        state.editorInteraction.kind !== "idle" &&
    ...
    const viewSpan =
      Math.max(window.innerWidth, window.innerHeight) * unitsPerPixel;
  ...
  private clearGroup(group: Group) {
    while (group.children.length > 0) {
      ...
      group.remove(child);
      child.traverse((node) => {
        const geometryHolder = node as typeof node & {
          geometry?: { dispose?: () => void };
        };
        geometryHolder.geometry?.dispose?.();

        const materialHolder = node as typeof node & {
